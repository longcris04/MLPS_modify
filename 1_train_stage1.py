...
def train_phase(args):
    ...
    for ep in range(args.max_epoches):
        ...
        for iter, (filename, data, label) in enumerate(train_progress_bar):
            img = data
            ...
            if ep > 2:
                ...
                enable_PDA = 0
            x, feature, y = model(img.to(device), enable_PDA)
            ...
            train_progress_bar.set_description(f"Epoch: {ep}/{args.max_epoches}. Loss: {loss.item()}. Avg_ep_EM: {avg_ep_EM}. Avg_ep_acc: {avg_ep_acc}.")
            # break
...
def test_phase(args):
    ...
    # save csv path
    csv_name = f"results_stage1_{args.dataset}.csv"
    ...
    cls_dice = score["Dice Coefficients"]
    mIoU = score["Mean IoU"]
    mDice = score["Mean Dice"]
    ...
    # append to dataframe
    new_row = pd.DataFrame({'iou_0':[iou_0], 'iou_1':[iou_1], 'iou_2':[iou_2], 'iou_3':[iou_3],
                             'miou':[mIoU],
...
if __name__ == '__main__':
    ...
    parser.add_argument("-e","--max_epoches", default=20, type=int)
    ...
    parser.add_argument("--weights", default='../ESFAN/init_weights/ilsvrc-cls_rna-a1_cls1000_ep-0001.params', type=str)
    ...
    parser.add_argument("--csv_dir", default='./results', type=str)
